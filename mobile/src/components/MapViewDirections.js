...
class MapViewDirections extends Component {
  ...
  coordsToWaypointsString = coords => {
    ...
    return coords.reduce(
      (acc, currentCoord) =>
        `${acc}|${currentCoord.latitude},${currentCoord.longitude}`,
      ""
    );
  };
  ...
  fetchRoute = (origin, destination, waypoints, apikey) => {
    ...
    const mode = "driving";
    const url = `https://maps.googleapis.com/maps/api/directions/json?origin=${origin}&destination=${destination}&waypoints=optimize:true${waypointsString}&key=${apikey}&mode=${mode}`;
